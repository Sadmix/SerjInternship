        ...
        switch (type) {
            ...
            case '%':  // Добавление оператора остатка от деления
                ...
                if (op2 != 0.0)
                    ...
                    printf("ошибка: деление на нуль\n");
                break;
            case 's': // sin: вычисление синуса верхнего элемента стека
                push(sin(pop()));
                break;
            case 'e': // exp: вычисление экспоненты верхнего элемента стека
                push(exp(pop()));
                break;
            case 'p': // pow: возведение в степень
                op2 = pop();
                push(pow(pop(), op2));
                break;
...
void clear_stack(void) {
    sp = 0; // сбрасываем указатель на вершину стека
    printf("Стек очищен\n");
}
...
int getop(char s[]) {
    ...
    if (c == '-' && !isdigit(c = getch())) {
        ...
        s[++i] = c; // это отрицательное число, продолжаем считывать
    } else {
        lastc = c;
    }
